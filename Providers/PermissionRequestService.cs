        ...
        try
        {
            foreach (var request in requests)
            {
                await using var command = new SqlCommand(
                    ...
                    "@PermissionDate, @PermissionDays, @PermissionType, @PermissionReason, @NIP, " +
                ...
                command.Parameters.Add(new SqlParameter("@PermissionDate", SqlDbType.Date)
                    { Value = GetPropertyValue(request, "PermissionDate") });
                command.Parameters.Add(new SqlParameter("@PermissionDays", SqlDbType.Float)
                    { Value = GetPropertyValue(request, "PermissionDays") });
                command.Parameters.Add(new SqlParameter("@PermissionType", SqlDbType.VarChar, 50)
                    { Value = GetPropertyValue(request, "PermissionType") });
